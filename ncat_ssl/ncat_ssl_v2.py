...
#====================================================================================================================
proto = 0 			# define the protocol: 0 = tls, 1 = dtls
logs = 1			# write logs to files: 1 = enable, 0 = disable 
test_iter = 2 		# number of iterations for each test_case
...
send_str = 'dnwencoqec  emciowmpom	qmANNSO!  SnusnU!@Ns! s1onia1imoinownx'+n
...
comm_20 = 'AT!="fsm"' + r
comm_21 = 'AT!="wakelock"' +r
comm_22 = 'cbe"setlock 0"' + r
...
def from_UE(ti, ts, tchannel, tprintout, check_response):
	...
	for tu in range(0, ti):
		...
		#print('lines list = ' + str(lineslist))
		for line in lineslist:
			#print('line in parser = ' + line)
			#line = line.decode().replace('\r', '').replace('\n', '')
			line = line.replace('\r', '').replace('\n', '')
			if tprintout == 1:
				super_print(line)
			#if line != '' and ts != '':
			if len(line) > 0 and len(ts) > 0:
				#print('ts = ' + ts)
				if line.find(ts) != -1:
					matchFound = 1
					break
...
def channelDetection():
	...
	if from_UE(10, '->', 'channel2_console', 0, 1) == 1:
	...
	while True:	#	nqueue.put(line)
		...
		if out == channel0_at:
			if uart0_enable == 1:
				...
				#if len(tmp_line) > 0:
					#print(str(nqueue)+' = '+str(tmp_line))
				for line in tmp_line:
					nqueue.put_nowait(line)
					if uart0_log == 1:
...
def ncat_info():
	...
	to_UE('channel0_at',comm_21)  # send wakelocks
	from_UE(5, 'OK', 'channel0_at', 1, 1)
...
def ssl_close_server():
	...
	for i in range(0,6):
		try:
			...
			super_print ('     Exception. Proc ' + str(i+1) + ' is already closed.')		
		time.sleep(.6)
...
def test_case_2(proto): # Sending data between UE and server
	...
	for i in range (1,7):
		...
		to_UE('channel0_at',spec_comm)  
		#from_UE(5, '', 'channel0_at', 1, 0)
		#=================================================================================
		super_print('Waiting for URC ...')
		from_UE(60, '+SQNSRING: '+str(i), 'channel0_at', 1, 1)
...
#====================================================================================================================
	...
	to_UE('channel2_console',comm_22)  # ============== send cbe"setlock 0"
	from_UE(2, '->', 'channel2_console', 1, 1)
#====================================================================================================================
